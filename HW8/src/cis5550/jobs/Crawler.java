...
public class Crawler {
        ...
        while(urlQueue.count()>0){
            urlQueue=urlQueue.flatMap(url->{
                ...
                try{
                    ...
                    int port = currentURL.getPort();String hostWithPort = (port != -1) ? host + ":" + port : host;Map<String, String> robotsRules = loadRobotsRules(kvs, hostWithPort);
                    ...
                    if (hostRow != null) {
                        ...
                        if (lastAccessTimeStr != null) {
                            System.out.println("时间是："+lastAccessTimeStr);
                            long lastAccessTime = Long.parseLong(lastAccessTimeStr);
                            if (currentTime - lastAccessTime < 1000) {
                                Thread.sleep(1000 - (currentTime - lastAccessTime));
                            }
                        }
                        else{
                            System.out.println("lastAccessTime is null");
                        }
                    }
                    ...
                    if (headResponseCode == 301 || headResponseCode == 302 || headResponseCode == 303 || headResponseCode == 307 || headResponseCode == 308) {
                        ...
                        System.out.println("from"+ url+"to"+location);
                        ...
                        if (connection.getResponseCode() == 200) {
                            ...
                            if (contentRow != null) {
                                ...
                                extractedUrls = HtmlParser.extractUrls(new String(pageContentBytes, StandardCharsets.UTF_8), currentURL.toString());
                            }
                        }
    ...
    public static void parseRobotsTxt(String robotsContent, Map<String, String> robotsRules) {
        ...
        for (String line : lines) {
            ...
            if (line.startsWith("User-agent:")) {
                ...
                if (line.startsWith("Disallow:")) {
                    ...
                    rulesList.add(new String[]{path, "Disallow"});
                } else if (line.startsWith("Allow:")) {
                    String path = line.split(":")[1].trim();
                    rulesList.add(new String[]{path, "Allow"});
                }
            }
        }
        rulesList.sort((r1, r2) -> Integer.compare(r2[0].length(), r1[0].length()));
        for (String[] rule : rulesList) {
            robotsRules.put(rule[0], rule[1]);
        }
    }

    // 检查 URL 路径是否被允许访问
    public static boolean isAllowedByRobots(Map<String, String> robotsRules, String path) {
        String matchedRule = null;
        String ruleType = null;
        for (Map.Entry<String, String> entry : robotsRules.entrySet()) {
            if (path.startsWith(entry.getKey())) {
                if (matchedRule == null || entry.getKey().length() > matchedRule.length()) {
                    matchedRule = entry.getKey();
                    ruleType = entry.getValue();
                }
            }
        }
        return ruleType == null || ruleType.equals("Allow");
    }
